...
class Info {
    ...
    public:
        ...
        int getChave() { return this->chave; } // retorna chave;
        string getValor() { return this->valor; } // retorna valor;
    private:
        int chave;
        string valor;
};
...
// metodo utilizado somente para depuracao/impressao
void ABB::percorreEmOrdem(){
    percorreEmOrdemAux(Raiz);
}

// metodo auxiliar da depuracao
...
// retorna o valor correspondente a chave, recebido de BuscaAux
string ABB::Busca(int& chave){
    Noh* Aux = BuscaAux(chave);
    if(Aux == NULL){
        // caso nao exista escreve inexistente no lugar do valor
        return "INEXISTENTE";
    }
    else{
        return Aux->dado;
...
// metodo auxiliar da busca que retorna o Noh, se não encontrado retorna NULL;
